(defpackage pprint-html
  (:use :cl :oliphaunt)
  (:export #:pprint-html))
(in-package :pprint-html)

(defvar *indent* 0)

(defgeneric pprint-html (expr subtype &rest metadata))

(defmethod pprint-html ((expr symbol) (subtype null) &rest _)
  (declare (ignore _))
  (let ((symbol-package (symbol-package expr)))
    (cond 
      ((eql (find-package :keyword) symbol-package)
       (pprint-html expr :keyword))
      ((member symbol-package (package-use-list *package*))
       (pprint-html expr :internal))
      (t (pprint-html expr :external)))))

(defun sigil (symbol)
  (let* ((name (symbol-name symbol))
         (first-elt (first-elt name))
         (last-elt (last-elt name)))
    (cond
      ((and (char= #\* first-elt)
            (char= #\* last-elt)) :*earmuff*)
      ((and (char= #\+ first-elt)
            (char= #\+ last-elt)) :+earmuff+)
      ((and (char= #\* last-elt)
            (< 2 (length name))
            (fboundp symbol)
            (fboundp (intern (subseq name 0 (1- (length name)))
                             (symbol-package symbol)))) :fun*)
      ;; Note: Among other things, / and * are NOT “Math” characters.
      ((or (find last-elt "/*-+÷×=⋅√<>≈≤≥" :test #'char=)
           (cl-unicode:has-property last-elt "Math"))
       :mathy)
      ((char= #\% last-elt) :sneaky)
      ((find #\_ name) :blank)
      ((find #\- name :test #'char=)
       (cond ((and (char= #\P last-elt)
                   (char= #\- (elt name (- (length name) 2))))
              :predicate)
             ((string-equal "DO-" name :end2 3) :do)
             ((string-equal "WITH-" name :end2 5) :with)
             ((string-equal "DEFINE-" name :end2 7) :define)
             (t t)))
      ((string-equal "DEF" name :end2 3) :define) 
      ((char= #\P last-elt) :predicate)
      (t t))))

(defun html-safe (string-designator)
  (let ((string (string string-designator)))
    (regex-replace "<" (regex-replace ">" (regex-replace "&" string
                                                         "&amp;")
                                      "&gt;") 
                   "&lt;")))

(defun symbol-pretty-name (symbol)
  (if (every #'upper-case-p (remove-if-not #'alpha-char-p (symbol-name symbol)))
      (html-safe (string-capitalize symbol))
      (format nil "<span class=\"quoted-symbol-name\">~a</span>" (html-safe symbol))))

(defmethod pprint-html ((expr symbol) (subtype (eql :keyword)) &rest _)
  (declare (ignore _))
  (html (a :title (concatenate 'string "Keyword " (symbol-pretty-name expr))
           :data-package "KEYWORD"
           :data-export-scope ":"
           :data-symbol-name (symbol-name expr))
    (html (span :class "export-scope") ":")
    (html (span :class "symbol-name") (symbol-pretty-name expr))))

(defmethod pprint-html ((fun symbol) (subtype (eql :funcall)) &rest _)
  (declare (ignore _))
  (html (a :href (symbol-link fun)
           :title (intro (or (documentation fun 'function)
                             (format nil "Function ~:(~a~)" fun)))
           :data-symbol-package (package-name (symbol-package fun))
           :data-export-scope (export-scope fun)
           :data-symbol-name (symbol-name fun))))

(defvar *clhs-root* "http://www.lispworks.com/reference/HyperSpec/")

(defvar *clhs-index* ;; Stolen from Slime hyperspec.el © 1997 Naggum Software; GPL
  (mapcan
   (lambda (row)
     (destructuring-bind (symbol page) row
       (list (find-symbol (string-upcase symbol) :common-lisp) page)))
...
(defun symbol-link (symbol)
  (if-let ((page (getf symbol *clhs-index*)))
    (concatenate 'string *clhs-root* "Body/" page)
    (concatenate 'string "/doc-strings/" (string-escape-uri-fragment symbol))))

(defmethod pprint-html ((expr symbol) (subtype (eql :internal)) &rest _)
  (declare (ignore _))
  (html (a :title (or (documentation expr 'variable)
                      (concatenate 'string "Symbol " (symbol-name expr)))
           :data-package (package-name (symbol-package expr))
           :data-symbol-name (symbol-name expr))
    (html (span :class "symbol-name")
      (html-safe (symbol-pretty-name expr)))))
...
(defun js-safe-string (string)
  (string-escape string (coerce #(#\Null #\Apostrophe #\" #\\
                                  #\Linefeed #\Return #\Tab
                                  #\Backspace #\Page)
                                'string)
                 #\\ #'escape-java))

(defun left-margin (line)
  (loop for i from 0
     for char = (elt line i)
     until (not (member char '(#\Tab #\Space) :test #'char=))
     finally (return i)))

(assert (= (left-margin "x") 0))
(assert (= (left-margin "   x") 3))

(defun first-paragraph (string)
  (block collect-paragraph
    (let* ((lines (split-sequence #\Newline string))
           (prefix (left-margin (if (endp (cdr lines)) 
                                    (first lines)
                                    (second lines)))))
      (loop for i from 0
         for line in lines
         until (or (every (rcurry #'member '(#\Tab #\Space) :test #'char=) line)
                   (and (plusp i)
                        (/= prefix (left-margin line))))
         finally (return-from collect-paragraph
                   (join #\Newline (subseq lines 0 i)))))))

(defun intro (string)
  (let ((first-paragraph (first-paragraph string)))
    (if (> 80 (length first-paragraph))
        (if (/= (length first-paragraph) (length string))
            (concatenate 'string first-paragraph " …")
            first-paragraph)
        (let ((s (subseq first-paragraph 0 200)))
          (if-let (stop (position #\. (subseq s 80 150) 
                                  :test #'char=))
            (concatenate 'string (subseq s 0 stop) " …")
            (if-let (space (position #\space (subseq s 80 150)
                                     :from-end t :test #'char=))
              (concatenate 'string (subseq s 0 space) " …")
              (concatenate 'string (subseq s 0 80) " …")))))))

(defun html-attr-safe (string)
  (string-escape string  (coerce #(#\Null #\Apostrophe #\" #\\
                                   #\Linefeed #\Return #\Tab
                                   #\Backspace #\Page)
                                 'string)
...
(defun replace-with-funref (target-string start end match-start match-end
                            reg-starts reg-ends)
  (declare (ignore match-start match-end start end))
  (let ((function-name (subseq target-string (elt reg-starts 0) (elt reg-ends 0))))
    (with-output-to-string (*standard-output*)
      (pprint-html (intern function-name) :function))))

(defun docstring->html (string &rest args)
  (let ((html (regex-replace-all 
               "_(.+)_" 
               (regex-replace-all 
                "`(.+)'" 
                (regex-replace-pairs 
    ...
    (when args
      (dolist (arg args)
        (setf html (regex-replace (concatenate 'string "(" (string-upcase arg) ")")
                                  html
                                  "<span class=\"symbol arg\">\\1</span>"))))))

(defmacro html ((element &rest attributes) &body body)
  `(progn
     (format t "~&~vt<~(~a~)~@[~{ ~(~a~)=\"~a\"~}~]>" *indent* ',element
             (list ,@(loop for (attr value) on attributes by #'cddr
                        appending (list (string-downcase attr)
                                        `(html-attr-safe ,value)))))
     (indented
       ,@body)
     (format t "~&~vt</~(~a~)>" *indent* ',element)))

(defun export-scope (symbol) ; FIXME
  (declare (ignore symbol))
  "::")

(defun label-with-ruby (label ruby &optional link)
  (check-type label string)
  (check-type ruby string)
  (html (span :class "rubied")
    (html (label) label)
    (html (ruby) ruby)))

(defmethod pprint-html ((expr cons) (subtype (eql :defun)) &rest _)
  (declare (ignore _))
  (html (div :class "sexp defun") 
    (let ((fun (car expr)))
      (html (span :class "car defun")
        (pprint-html fun :funcall))
      (let ((λ-list (sb-introspect:function-lambda-list fun))
            (portion :positional))
        (block print-lambda-list
          (loop
             (let ((binding (pop λ-list)))
               (when (member binding '(&body &whole))
                 (return-from print-lambda-list
                   (label-with-ruby (symbol-pretty-name (pop bindings))
                                    binding)))
               (when (eql '&environment binding)
                 (return-from print-lambda-list
                   (label-with-ruby arg (symbol-pretty-name (pop bindings))
                                    binding)))
               (when (member binding lambda-list-keywords)
                 ())
               
               ))))))
  (format t "~&~vt</div>" *indent*))

(defmethod pprint-html ((expr cons) (subtype (eql :list)))
  (format t "~&~vt<ol class=\"sexp list\">" *indent*)
  (indented (dolist (element list)
              (format t "~&~vt<li>")
              (pprint-html element)
              (format t "~&~vt</li>")))
  (format t "~&~vt</ol>" *indent*))

(defmethod  pprint-html ((expr cons) (subtype null) &rest _)
  (cond
    ((circular-list-p expr)
     (pprint-html expr :circular))
    ((eql 'quote (car expr))
     (pprint-html expr :quoted))
    ((eql 'sb-int:quasiquote (car expr))
     (pprint-html expr :back-quoted))
    ((eql 'sb-impl::comma (car expr))
     (pprint-html (rest expr) :unquoted))
    ((macro-function (car expr))
     (pprint-html expr :macro))
    ((fboundp (car expr))
     (pprint-html expr :funcall))
    (t (pprint-html expr :list))))

(defmethod pprint-html ((expr symbol) (subtype (eql :funcall)) &rest _)
  (indented 
   (let ((name (symbol-pretty-name expr)))
     (format t "~&~vt<a title=\"~a\"
~vt   data-package=\"~a\" 
~vt   data-symbol-name=\"~a\"><span class=\"symbol-name\">~a</a>" 
             *indent* (html-safe (or (documentation expr 'function)
                                     (concatenate 'string "Symbol " (symbol-name expr))))
             *indent* (html-safe (package-name (symbol-package expr)))
             *indent* (symbol-name expr) (html-safe name)))))
